...
export default function Navbar() {
    ...
    const handleSearch = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (input.value.trim()) {
            router.push(`/search?q=${encodeURIComponent(input.value)}&type=${encodeURIComponent(searchType)}`)
        }
    }
    ...
    return (
        <nav className="bg-[#121212] text-white py-3 px-4 md:px-8 flex items-center justify-between sticky top-0 z-50 border-b border-gray-800">
            <div className="flex items-center gap-4 w-full max-w-7xl mx-auto">
                {/* Logo */}
                <Link href="/" className="bg-primary text-black font-bold px-3 py-1 rounded-md text-sm md:text-base tracking-tighter shrink-0">
                    DBFPN
                </Link>
                ...
                <div className="flex-1 flex items-center mx-4">
                    <div className="relative flex items-center w-full">
                        ...
                        >
                            ...
                            />
                            <button type="submit" className="h-8 bg-white text-gray-500 px-3 rounded-r-sm hover:text-black transition-colors shrink-0 flex items-center justify-center">
                                <Search size={18} />
                            </button>
                        </form>
                    </div>
                ...
                <div className="hidden md:flex items-center gap-6 text-sm font-medium text-gray-300 shrink-0">
                    ...
                    <Link href="/top-100" className="hover:text-white transition-colors">Top 100</Link>
                    <Link href="/dashboard/user" className="hover:text-white transition-colors">Dasbor</Link>
                    <Link href="/signin" className="hover:text-white transition-colors">Masuk</Link>
                </div>
            </div>
        </nav>
    )
